...
public class TileEntityBlastingChamber extends TileEntity implements ITickable
{
	...
	{
		return this.hasCustomName() ? new TextComponentString(this.customName) : new TextComponentTranslation("Blasting Chamber");
	}
	...
	public static boolean isBurning(TileEntityBlastingChamber te) 
	{
		return te.getCurrentBurnTime() > 0;
	}
	...
	{	
		...
		{
			--this.burnTime;
			BlastingChamber.setState(true, world, pos);
		}
		...
		else
		{
			...
			{
				ItemStack output = BlastingChamberRecipes.getInstance().getBlastingResult(inputs[0], inputs[1]);
				if(!output.isEmpty())
				{
					smelting = output;
					cookTime++;
	...
	private boolean canSmelt() 
	{
		...
		{
			ItemStack result = BlastingChamberRecipes.getInstance().getBlastingResult((ItemStack)this.handler.getStackInSlot(0), (ItemStack)this.handler.getStackInSlot(1));	
	...
	public static int getItemBurnTime(ItemStack fuel) 
	{
		...
		else 
		{
			...
			if (item == Items.REEDS) return 200;
	...
	//removing this for setters above
