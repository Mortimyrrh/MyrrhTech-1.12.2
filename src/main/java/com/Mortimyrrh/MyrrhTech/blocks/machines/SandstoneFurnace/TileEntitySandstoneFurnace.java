...
public class TileEntitySandstoneFurnace extends TileEntity implements ITickable
{
	...
	{
		return this.hasCustomName() ? new TextComponentString(this.customName) : new TextComponentTranslation("Sandstone Furnace");
	}
	...
	public static boolean isBurning(TileEntitySandstoneFurnace te) 
	{
	...
	{	
		...
		else
		{
			...
			{
				ItemStack output = SandstoneFurnaceRecipes.getInstance().getSandstoneResult(inputs[0], inputs[1]);
				if(!output.isEmpty())
				{
					smelting = output;
					cookTime++;
	...
	private boolean canSmelt() 
	{
		...
		{
			ItemStack result = SandstoneFurnaceRecipes.getInstance().getSandstoneResult((ItemStack)this.handler.getStackInSlot(0), (ItemStack)this.handler.getStackInSlot(1));	
